...
class BaytScraper(Scraper):
    ...
    def _fetch_jobs(self, query: str, page: int) -> list | None:
        ...
        try:
            ...
            url = f"{self.base_url}/en/international/jobs/{query_formatted}-jobs/?page={page}"
    ...
    def _extract_job_info(self, job: BeautifulSoup) -> JobPost | None:
        ...
        # Extract company name using updated class selectors
        company_tag = job.find("a", class_="t-bold")
        if not company_tag:
            company_tag = job.find("div", class_="job-company-location-wrapper")
            if company_tag:
                company_tag = company_tag.find("a")
        company_name = company_tag.get_text(strip=True) if company_tag else None

        # Extract location
        location_wrapper = job.find("div", class_="job-company-location-wrapper")
        location = None
        if location_wrapper:
            loc_tag = location_wrapper.find("div", class_="t-mute")
            if loc_tag:
        ...
        descr_div = job.find("div", class_="jb-descr")
        description = None
        if descr_div:
            description = descr_div.get_text(" ", strip=True)
            if "summary:" in description.lower():
                # Clean up prefix summary label if exists
                description = re.sub(r"^(?:AI\s+)?Summary:\s*", "", description, flags=re.IGNORECASE).strip()
